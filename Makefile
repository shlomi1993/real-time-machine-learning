...
main: bindir $(MNIST_ML_ROOT)/main.cc $(LIB_DATA)
	$(CC) -fPIC $(CFLAGS) -o $(MNIST_ML_ROOT)/main $(MNIST_ML_ROOT)/main.cc -I$(INCLUDE_DIR) -L$(MNIST_ML_ROOT)/lib -ldata  # Edited: Compile and link `main`

clean:
	rm -r $(MNIST_ML_ROOT)/lib
	rm -r $(MNIST_ML_ROOT)/obj
	rm -f $(MNIST_ML_ROOT)/main  # Edited: Added removal of `main` executable
